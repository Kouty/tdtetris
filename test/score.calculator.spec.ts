...
describe('ScoreCalculator', () => {
    ...
    it('should add 10 for 1 row cleared', () => {
        calculator.addPointsForClear(1);

        expect(calculator.score).toBe(10);
    });

    it('should add 10 + 20 for 2 rows cleared', () => {
        calculator.addPointsForClear(2);

        expect(calculator.score).toBe(30);
    });

    it('should add the sum of cleared rows * 10 points for N rows cleared', () => {
        calculator.addPointsForClear(4);

        expect(calculator.score).toBe(100);
    });
